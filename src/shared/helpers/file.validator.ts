...
import { FILE_EXCEEDS_SIZE, INVALID_FILE_TYPE } from '@shared/constants/SystemMessages';
...
export class FileValidator implements PipeTransform {
    ...
    async transform(value: Express.Multer.File, metadata: ArgumentMetadata) {
        if (!value) throw new CustomHttpException('No file provided', HttpStatus.BAD_REQUEST);
    ...
    private validateFileSize(size: number) {
        if (size > this.options.maxSize) {
            throw new CustomHttpException(FILE_EXCEEDS_SIZE(this.options.maxSize / (1024 * 1024)), HttpStatus.BAD_REQUEST);
        }
    }
